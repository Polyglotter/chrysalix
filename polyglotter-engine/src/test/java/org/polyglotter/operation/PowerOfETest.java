...
public final class PowerOfETest implements TestConstants {

    private PowerOfE operation;

    @Before
    public void beforeEach() {
        this.operation = new PowerOfE( ID, TRANSFORM_ID );
    }
    ...
    public void shouldCalculateDoubleTerm() throws PolyglotterException {
        ...
        assertThat( this.operation.result(), is( ( Number ) Math.exp( DOUBLE_1_VALUE ) ) );
    }

    @Test
    public void shouldCalculateFloatTerm() throws PolyglotterException {
        this.operation.add( FLOAT_1 );
        assertThat( this.operation.result(), is( ( Number ) Math.exp( FLOAT_1_VALUE ) ) );
    }

    @Test
    public void shouldCalculateIntegerTerm() throws PolyglotterException {
        this.operation.add( INT_1 );
        assertThat( this.operation.result(), is( ( Number ) Math.exp( INT_1_VALUE ) ) );
    }

    @Test
    public void shouldCalculateLongTerm() throws PolyglotterException {
        this.operation.add( LONG_1 );
        assertThat( this.operation.result(), is( ( Number ) Math.exp( LONG_1_VALUE ) ) );
    }
