...
public final class PowerOfETest {
    ...
    public void beforeEach() {
        this.operation = new PowerOfE( TestConstants.ID, TestConstants.TRANSFORM_ID );
    }
    ...
    public void shouldCalculateDoubleTerm() throws PolyglotterException {
        ...
        assertThat( this.operation.result(), is( ( Number ) Math.exp( TestConstants.DOUBLE_1_VALUE ) ) );
    }

    @Test
    public void shouldCalculateFloatTerm() throws PolyglotterException {
        this.operation.add( TestConstants.FLOAT_1_TERM );
        assertThat( this.operation.result(), is( ( Number ) Math.exp( TestConstants.FLOAT_1_VALUE ) ) );
    }

    @Test
    public void shouldCalculateIntegerTerm() throws PolyglotterException {
        this.operation.add( TestConstants.INT_1_TERM );
        assertThat( this.operation.result(), is( ( Number ) Math.exp( TestConstants.INT_1_VALUE ) ) );
    }

    @Test
    public void shouldCalculateLongTerm() throws PolyglotterException {
        this.operation.add( TestConstants.LONG_1_TERM );
        assertThat( this.operation.result(), is( ( Number ) Math.exp( TestConstants.LONG_1_VALUE ) ) );
    }

    @Test
    public void shouldHaveAbbreviation() {
        assertThat( this.operation.descriptor().abbreviation(), is( "exp" ) );
    }
