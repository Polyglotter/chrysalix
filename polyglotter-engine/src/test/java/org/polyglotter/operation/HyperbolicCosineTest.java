...
public final class HyperbolicCosineTest implements TestConstants {

    private HyperbolicCosine operation;

    @Before
    public void beforeEach() {
        this.operation = new HyperbolicCosine( ID, TRANSFORM_ID );
    }
    ...
    public void shouldCalculateDoubleTerm() throws PolyglotterException {
        ...
        assertThat( this.operation.result(), is( ( Number ) Math.cosh( DOUBLE_1_VALUE ) ) );
    }

    @Test
    public void shouldCalculateFloatTerm() throws PolyglotterException {
        this.operation.add( FLOAT_1 );
        assertThat( this.operation.result(), is( ( Number ) Math.cosh( FLOAT_1_VALUE ) ) );
    }

    @Test
    public void shouldCalculateIntegerTerm() throws PolyglotterException {
        this.operation.add( INT_1 );
        assertThat( this.operation.result(), is( ( Number ) Math.cosh( INT_1_VALUE ) ) );
    }

    @Test
    public void shouldCalculateLongTerm() throws PolyglotterException {
        this.operation.add( LONG_1 );
        assertThat( this.operation.result(), is( ( Number ) Math.cosh( LONG_1_VALUE ) ) );
    }
