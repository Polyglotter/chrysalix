...
public final class CubeRootTest implements TestConstants {

    private CubeRoot operation;

    @Before
    public void beforeEach() {
        this.operation = new CubeRoot( ID, TRANSFORM_ID );
    }
    ...
    public void shouldCalculateDoubleTerm() throws PolyglotterException {
        ...
        assertThat( this.operation.result(), is( ( Number ) Math.cbrt( DOUBLE_1_VALUE ) ) );
    }

    @Test
    public void shouldCalculateFloatTerm() throws PolyglotterException {
        this.operation.add( FLOAT_1 );
        assertThat( this.operation.result(), is( ( Number ) Math.cbrt( FLOAT_1_VALUE ) ) );
    }

    @Test
    public void shouldCalculateIntegerTerm() throws PolyglotterException {
        this.operation.add( INT_1 );
        assertThat( this.operation.result(), is( ( Number ) Math.cbrt( INT_1_VALUE ) ) );
    }

    @Test
    public void shouldCalculateLongTerm() throws PolyglotterException {
        this.operation.add( LONG_1 );
        assertThat( this.operation.result(), is( ( Number ) Math.cbrt( LONG_1_VALUE ) ) );
    }
