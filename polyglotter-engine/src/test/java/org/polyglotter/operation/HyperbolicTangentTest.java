...
public final class HyperbolicTangentTest implements TestConstants {

    private HyperbolicTangent operation;

    @Before
    public void beforeEach() {
        this.operation = new HyperbolicTangent( ID, TRANSFORM_ID );
    }
    ...
    public void shouldCalculateDoubleTerm() throws PolyglotterException {
        ...
        assertThat( this.operation.result(), is( ( Number ) Math.tanh( DOUBLE_1_VALUE ) ) );
    }

    @Test
    public void shouldCalculateFloatTerm() throws PolyglotterException {
        this.operation.add( FLOAT_1 );
        assertThat( this.operation.result(), is( ( Number ) Math.tanh( FLOAT_1_VALUE ) ) );
    }

    @Test
    public void shouldCalculateIntegerTerm() throws PolyglotterException {
        this.operation.add( INT_1 );
        assertThat( this.operation.result(), is( ( Number ) Math.tanh( INT_1_VALUE ) ) );
    }

    @Test
    public void shouldCalculateLongTerm() throws PolyglotterException {
        this.operation.add( LONG_1 );
        assertThat( this.operation.result(), is( ( Number ) Math.tanh( LONG_1_VALUE ) ) );
    }
