...
public final class ITJava2JavaTransformation extends BaseTest {
    ...
    public void installCategory() throws Exception {
        polyglotter().metamodelManager().install( CATEGORY );
    }

    private Metamodel metamodel() throws Exception {
        return polyglotter().metamodelManager().metamodel( METAMODEL_ID );
    }

    @Before
    public void registerTestRepository() throws Exception {
        polyglotter().metamodelManager().registerMetamodelRepository( INTEGRATION_TEST_METAMODEL_REPOSITORY_URL );
    }
    ...
    public void shouldMapFieldToField() throws Exception {
        ...
        final Model sourceModel = polyglotter().importModel( sourceFile, "test", metamodel() );
        ...
        final Model targetModel = polyglotter().importModel( targetFile, "test", metamodel() );
