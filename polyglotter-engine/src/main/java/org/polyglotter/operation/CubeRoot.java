 ...
 */
public final class CubeRoot extends AbstractOperation< Double > {

    /**
     * The input term descriptor.
     */
    public static final ValueDescriptor< Number > TERM_DESCRIPTOR =
        TransformationFactory.createWritableBoundedOneValueDescriptor( TransformationFactory.createId( CubeRoot.class, "input" ),
                                                                       PolyglotterI18n.cubeRootOperationInputDescription.text(),
                                                                       PolyglotterI18n.cubeRootOperationInputName.text(),
                                                                       Number.class );
    ...
    public static final OperationDescriptor< Double > DESCRIPTOR =
        new AbstractOperationDescriptor< Double >( TransformationFactory.createId( CubeRoot.class ),
                                                   PolyglotterI18n.cubeRootOperationDescription.text(),
                                                   PolyglotterI18n.cubeRootOperationName.text(),
                                                   Double.class,
                                                   INPUT_DESCRIPTORS ) {
            ...
            public Operation< Double > newInstance( final Transformation transformation ) {
                return new CubeRoot( transformation );
            }
     ...
     */
    CubeRoot( final Transformation transformation ) {
    ...
    protected Double calculate() throws PolyglotterException {
        ...
        return Math.cbrt( value.doubleValue() );
    }
        ...
        if ( inputs().size() != 1 ) {
            final ValidationProblem problem =
                TransformationFactory.createError( transformationId(),
                                                   PolyglotterI18n.cubeRootOperationMustHaveOneTerm.text( transformationId() ) );
            ...
            try {
                ...
                if ( !( value instanceof Number ) ) {
                    final ValidationProblem problem =
                        TransformationFactory.createError( transformationId(),
                                                           PolyglotterI18n.cubeRootOperationInvalidTermType.text( transformationId() ) );
                    problems().add( problem );
                }
            } catch ( final PolyglotterException e ) {
                final ValidationProblem problem =
