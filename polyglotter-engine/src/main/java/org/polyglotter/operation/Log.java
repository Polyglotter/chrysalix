...
/**
 * Calculates the natural logarithm (base <i>e</i>).
 * 
 * @see Math#log(double)
 */
public final class Log extends AbstractOperation< Double > {

    /**
     * The input term descriptor.
     */
    public static final ValueDescriptor< Number > TERM_DESCRIPTOR =
        TransformationFactory.createWritableBoundedOneValueDescriptor( TransformationFactory.createId( Log.class, "input" ),
                                                                       PolyglotterI18n.logOperationInputDescription.text(),
                                                                       PolyglotterI18n.logOperationInputName.text(),
                                                                       Number.class );
    ...
    public static final OperationDescriptor< Double > DESCRIPTOR =
        new AbstractOperationDescriptor< Double >( TransformationFactory.createId( Log.class ),
                                                   PolyglotterI18n.logOperationDescription.text(),
                                                   PolyglotterI18n.logOperationName.text(),
                                                   Double.class,
                                                   INPUT_DESCRIPTORS ) {
     ...
     */
    Log( final Transformation transformation ) {
        ...
        if ( inputs().size() != 1 ) {
            final ValidationProblem problem =
                TransformationFactory.createError( transformationId(),
                                                   PolyglotterI18n.logOperationMustHaveOneTerm.text( transformationId() ) );
            ...
            try {
                ...
                if ( !( value instanceof Number ) ) {
                    final ValidationProblem problem =
                        TransformationFactory.createError( transformationId(),
                                                           PolyglotterI18n.logOperationInvalidTermType.text( transformationId() ) );
                    problems().add( problem );
                }
            } catch ( final PolyglotterException e ) {
                final ValidationProblem problem =
