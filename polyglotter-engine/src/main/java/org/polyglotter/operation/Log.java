 ...
 * @see Math#log(double)
 */
public class Log extends BaseOperation< Double > {
    ...
    public String description() {
        return PolyglotterI18n.logOperationDescription.text();
    }
    ...
    @Override
    public String name() {
        return PolyglotterI18n.logOperationName.text();
    }
    ...
    protected void validate() {
        ...
        if ( terms().size() != 1 ) {
            final ValidationProblem problem =
                GrammarFactory.createError( id(), PolyglotterI18n.logOperationMustHaveOneTerm.text( id() ) );
            ...
            if ( !( value instanceof Number ) ) {
                final ValidationProblem problem =
                    GrammarFactory.createError( id(),
                                                PolyglotterI18n.logOperationInvalidTermType.text( id(),
                                                                                                  terms().get( 0 ).id() ) );
                problems().add( problem );
            }
        }
