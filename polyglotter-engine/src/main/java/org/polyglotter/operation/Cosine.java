 ...
 * @see Math#cos(double)
 */
public class Cosine extends BaseOperation< Double > {

    /**
     * @param id
     *        the cosine operation's unique identifier (cannot be <code>null</code>)
     * @param transformId
     *        the owning transform identifier (cannot be <code>null</code>)
     * @throws IllegalArgumentException
     *         if any inputs are <code>null</code>
     */
    public Cosine( final QName id,
    ...
    public String description() {
        return PolyglotterI18n.cosineOperationDescription.text();
    }
    ...
    @Override
    public String name() {
        return PolyglotterI18n.cosineOperationName.text();
    }
    ...
    protected void validate() {
        ...
        if ( terms().size() != 1 ) {
            final ValidationProblem problem =
                GrammarFactory.createError( id(), PolyglotterI18n.cosineOperationMustHaveOneTerm.text( id() ) );
            ...
            if ( !( value instanceof Number ) ) {
                final ValidationProblem problem =
                    GrammarFactory.createError( id(),
                                                PolyglotterI18n.cosineOperationInvalidTermType.text( id(),
                                                                                                     terms().get( 0 ).id() ) );
                problems().add( problem );
            }
        }
