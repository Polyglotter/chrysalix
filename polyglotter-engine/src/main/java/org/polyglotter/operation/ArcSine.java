 ...
 */
public final class ArcSine extends AbstractOperation< Double > {

    /**
     * The input term descriptor.
     */
    public static final ValueDescriptor< Number > TERM_DESCRIPTOR =
        TransformationFactory.createWritableBoundedOneValueDescriptor( TransformationFactory.createId( ArcSine.class, "input" ),
                                                                       PolyglotterI18n.arcSineOperationInputDescription.text(),
                                                                       PolyglotterI18n.arcSineOperationInputName.text(),
                                                                       Number.class );
    ...
    public static final OperationDescriptor< Double > DESCRIPTOR =
        new AbstractOperationDescriptor< Double >( TransformationFactory.createId( ArcSine.class ),
                                                   PolyglotterI18n.arcSineOperationDescription.text(),
                                                   PolyglotterI18n.arcSineOperationName.text(),
                                                   Double.class,
                                                   INPUT_DESCRIPTORS ) {
            ...
            public Operation< Double > newInstance( final Transformation transformation ) {
                return new ArcSine( transformation );
            }
     ...
     */
    ArcSine( final Transformation transformation ) {
        ...
        if ( inputs().size() != 1 ) {
            final ValidationProblem problem =
                TransformationFactory.createError( transformationId(),
                                                   PolyglotterI18n.arcSineOperationMustHaveOneTerm.text( transformationId() ) );
            ...
            try {
                ...
                if ( !( value instanceof Number ) ) {
                    final ValidationProblem problem =
                        TransformationFactory.createError( transformationId(),
                                                           PolyglotterI18n.arcSineOperationInvalidTermType.text( transformationId() ) );
                    problems().add( problem );
                }
            } catch ( final PolyglotterException e ) {
                final ValidationProblem problem =
