...
public final class Ceiling extends AbstractOperation< Integer > {
    ...
    public static final ValueDescriptor< Integer > DESCRIPTOR =
        TransformationFactory.createReadOnlyBoundedOneValueDescriptor( TransformationFactory.createId( Ceiling.class.getSimpleName() ),
                                                                       PolyglotterI18n.ceilingOperationDescription.text(),
                                                                       PolyglotterI18n.ceilingOperationName.text(),
                                                                       Integer.class );
    ...
    private static final List< ValueDescriptor< Number >> INPUT_DESCRIPTORS =
        ...
        Collections.singletonList(
                   TransformationFactory.createWritableBoundedOneValueDescriptor( TransformationFactory.createId( Ceiling.class.getSimpleName() ),
                                                                                  PolyglotterI18n.ceilingOperationDescription.text(),
                                                                                  PolyglotterI18n.ceilingOperationName.text(),
                                                                                  Number.class ) );
        ...
        if ( inputs().size() != 1 ) {
            final ValidationProblem problem =
                TransformationFactory.createError( transformationId(),
                                                   PolyglotterI18n.ceilingOperationMustHaveOneTerm.text( transformationId() ) );
            ...
            try {
                ...
                if ( !( value instanceof Number ) ) {
                    final ValidationProblem problem =
                        TransformationFactory.createError( transformationId(),
                                                           PolyglotterI18n.ceilingOperationInvalidTermType.text( transformationId() ) );
                    problems().add( problem );
                }
            } catch ( final PolyglotterException e ) {
                final ValidationProblem problem =
