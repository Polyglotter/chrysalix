 ...
 * @see Math#tan(double)
 */
public class Tangent extends BaseOperation< Double > {

    /**
     * @param id
     *        the tangent operation's unique identifier (cannot be <code>null</code>)
     * @param transformId
     *        the owning transform identifier (cannot be <code>null</code>)
     * @throws IllegalArgumentException
     *         if any inputs are <code>null</code>
     */
    public Tangent( final QName id,
    ...
    public String description() {
        return PolyglotterI18n.tangentOperationDescription.text();
    }
    ...
    @Override
    public String name() {
        return PolyglotterI18n.tangentOperationName.text();
    }
    ...
    protected void validate() {
        ...
        if ( terms().size() != 1 ) {
            final ValidationProblem problem =
                GrammarFactory.createError( id(), PolyglotterI18n.tangentOperationMustHaveOneTerm.text( id() ) );
            ...
            if ( !( value instanceof Number ) ) {
                final ValidationProblem problem =
                    GrammarFactory.createError( id(),
                                                PolyglotterI18n.tangentOperationInvalidTermType.text( id(),
                                                                                                      terms().get( 0 ).id() ) );
                problems().add( problem );
            }
        }
