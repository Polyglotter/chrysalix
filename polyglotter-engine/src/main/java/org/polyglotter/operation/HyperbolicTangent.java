 ...
 */
public final class HyperbolicTangent extends AbstractOperation< Double > {

    /**
     * The input term descriptor.
     */
    public static final ValueDescriptor< Number > TERM_DESCRIPTOR =
        TransformationFactory.createWritableBoundedOneValueDescriptor( TransformationFactory.createId( HyperbolicTangent.class, "input" ),
                                                                       PolyglotterI18n.hyperbolicTangentOperationInputDescription.text(),
                                                                       PolyglotterI18n.hyperbolicTangentOperationInputName.text(),
                                                                       Number.class );
    ...
    public static final OperationDescriptor< Double > DESCRIPTOR =
        new AbstractOperationDescriptor< Double >( TransformationFactory.createId( HyperbolicTangent.class ),
                                                   PolyglotterI18n.hyperbolicTangentOperationDescription.text(),
                                                   PolyglotterI18n.hyperbolicTangentOperationName.text(),
                                                   Double.class,
                                                   INPUT_DESCRIPTORS ) {
            ...
            public Operation< Double > newInstance( final Transformation transformation ) {
                return new HyperbolicTangent( transformation );
            }
     ...
     */
    HyperbolicTangent( final Transformation transformation ) {
        ...
        if ( inputs().size() != 1 ) {
            final ValidationProblem problem =
                TransformationFactory.createError( transformationId(),
                                                   PolyglotterI18n.hyperbolicTangentOperationMustHaveOneTerm.text( transformationId() ) );
            ...
            try {
                ...
                if ( !( value instanceof Number ) ) {
                    final ValidationProblem problem =
                        TransformationFactory.createError( transformationId(),
                                                           PolyglotterI18n.hyperbolicTangentOperationInvalidTermType.text( transformationId() ) );
                    problems().add( problem );
                }
            } catch ( final PolyglotterException e ) {
                final ValidationProblem problem =
