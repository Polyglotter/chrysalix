 ...
 * @see Math#exp(double)
 */
public class PowerOfE extends BaseOperation< Double > {
     ...
     */
    public PowerOfE( final QName id,
    ...
    public String description() {
        return PolyglotterI18n.powerOfEOperationDescription.text();
    }
    ...
    @Override
    public String name() {
        return PolyglotterI18n.powerOfEOperationName.text();
    }
    ...
    protected void validate() {
        ...
        if ( terms().size() != 1 ) {
            final ValidationProblem problem =
                GrammarFactory.createError( id(), PolyglotterI18n.powerOfEOperationMustHaveOneTerm.text( id() ) );
            ...
            if ( !( value instanceof Number ) ) {
                final ValidationProblem problem =
                    GrammarFactory.createError( id(),
                                                PolyglotterI18n.powerOfEOperationInvalidTermType.text( id(),
                                                                                                       terms().get( 0 ).id() ) );
                problems().add( problem );
            }
        }
