 ...
 */
public final class ArcCosine extends AbstractOperation< Double > {

    /**
     * The input term descriptor.
     */
    public static final ValueDescriptor< Number > TERM_DESCRIPTOR =
        TransformationFactory.createWritableBoundedOneValueDescriptor( TransformationFactory.createId( ArcCosine.class, "input" ),
                                                                       PolyglotterI18n.arcCosineOperationInputDescription.text(),
                                                                       PolyglotterI18n.arcCosineOperationInputName.text(),
                                                                       Number.class );
    ...
    public static final OperationDescriptor< Double > DESCRIPTOR =
        new AbstractOperationDescriptor< Double >( TransformationFactory.createId( ArcCosine.class ),
                                                   PolyglotterI18n.arcCosineOperationDescription.text(),
                                                   PolyglotterI18n.arcCosineOperationName.text(),
                                                   Double.class,
                                                   INPUT_DESCRIPTORS ) {
            ...
            public Operation< Double > newInstance( final Transformation transformation ) {
                return new ArcCosine( transformation );
            }
     ...
     */
    ArcCosine( final Transformation transformation ) {
        ...
        if ( inputs().size() != 1 ) {
            final ValidationProblem problem =
                TransformationFactory.createError( transformationId(),
                                                   PolyglotterI18n.arcCosineOperationMustHaveOneTerm.text( transformationId() ) );
            ...
            try {
                ...
                if ( !( value instanceof Number ) ) {
                    final ValidationProblem problem =
                        TransformationFactory.createError( transformationId(),
                                                           PolyglotterI18n.arcCosineOperationInvalidTermType.text( transformationId() ) );
                    problems().add( problem );
                }
            } catch ( final PolyglotterException e ) {
                final ValidationProblem problem =
