...
/**
 * The XSD dependency processor for the modelspace.
 */
public final class XsdDependencyProcessor implements DependencyProcessor {

    private static final Logger LOGGER = Logger.logger( XsdI18n.class );
    ...
    public String process( final String dataPath,
        ...
        try {
            ...
            while ( itr.hasNext() ) {
                ...
                if ( dependenciesNode == null ) {
                    if ( modelNode.hasNode( ModelspaceLexicon.Model.DEPENDENCIES ) ) {
                        dependenciesNode = modelNode.getNode( ModelspaceLexicon.Model.DEPENDENCIES );
                    } else {
                        dependenciesNode = modelNode.addNode( ModelspaceLexicon.Model.DEPENDENCIES,
                                                              ModelspaceLexicon.Model.DEPENDENCIES );
                ...
                @SuppressWarnings( "null" ) final Node dependencyNode =
                    dependenciesNode.addNode( ModelspaceLexicon.Dependency.DEPENDENCY, ModelspaceLexicon.Dependency.DEPENDENCY );

                // set input property
                final Property locationProp = kid.getProperty( XsdLexicon.SCHEMA_LOCATION );
                final String location = locationProp.getString();
                dependencyNode.setProperty( ModelspaceLexicon.Dependency.SOURCE_REFERENCE_PROPERTY, new String[] { location } );
                ...
                if ( pathIsRelative( path ) ) {
                    ...
                    dependencyNode.setProperty( ModelspaceLexicon.Dependency.PATH, fullModelPath );
                    ...
                    if ( modelNode.hasProperty( ModelspaceLexicon.Model.EXTERNAL_LOCATION ) ) {
                        String extLocation = modelNode.getProperty( ModelspaceLexicon.Model.EXTERNAL_LOCATION ).getString();
                        ...
                        dependencyNode.setProperty( ModelspaceLexicon.Dependency.PATH, dependencyModelPath );
            ...
            if ( !pathsToMissingDependencies.isEmpty() ) {
                uploadMissingDependencies( dataPath, modelNode, pathsToMissingDependencies, modelspace, persistArtifacts );
            }

            modelNode.getSession().save();
            return dependenciesNode.getPath();
        } catch ( final Exception e ) {
            if ( e instanceof ModelspaceException ) throw ( ModelspaceException ) e;
    ...
    void uploadMissingDependencies( final String dataPath,
        ...
        assert ( modelspace != null );

        if ( !modelNode.hasProperty( ModelspaceLexicon.Model.EXTERNAL_LOCATION )
             || !modelNode.hasProperty( ModelspaceLexicon.Model.METAMODEL )
             || missingDependencies.isEmpty() ) {
            return;
        }

        final String modelName = modelNode.getName();
        final String id = modelNode.getProperty( ModelspaceLexicon.Model.METAMODEL ).getString();
        final Metamodel metamodel = modelspace.metamodelManager().metamodel( id );

        String externalLocation = modelNode.getProperty( ModelspaceLexicon.Model.EXTERNAL_LOCATION ).getString();
        ...
        for ( final MissingDependency missingDependency : missingDependencies ) {
            ...
            try {
                ...
                final String dependencyArtifactPath = modelspace.importData( new URL( extPath ).openStream(), dataLocation );

                // create model
                LOGGER.debug( "Generating model for XSD dependency of model '%s' from path '%s'", modelName, modelPath );
                ( ( ModelspaceImpl ) modelspace ).generateModel( dependencyArtifactPath, modelPath, metamodel, persistArtifacts );
            } catch ( final Exception e ) {
