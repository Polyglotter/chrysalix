...
public final class Floor extends AbstractOperation< Integer > {
    ...
    public static final ValueDescriptor< Number > TERM_DESCRIPTOR =
        TransformationFactory.createWritableBoundedOneValueDescriptor( TransformationFactory.createId( Floor.class, "input" ),
                                                                       ChrysalixI18n.floorOperationInputDescription.text(),
                                                                       ChrysalixI18n.floorOperationInputName.text(),
                                                                       Number.class );
    ...
    public static final OperationDescriptor< Integer > DESCRIPTOR =
        new AbstractOperationDescriptor< Integer >( TransformationFactory.createId( Floor.class ),
                                                    ChrysalixI18n.floorOperationDescription.text(),
                                                    ChrysalixI18n.floorOperationName.text(),
                                                    Integer.class,
                                                    INPUT_DESCRIPTORS ) {
        ...
        if ( inputs().size() != 1 ) {
            final ValidationProblem problem =
                TransformationFactory.createError( transformationId(),
                                                   ChrysalixI18n.floorOperationMustHaveOneTerm.text( transformationId() ) );
            ...
            try {
                ...
                if ( !( value instanceof Number ) ) {
                    final ValidationProblem problem =
                        TransformationFactory.createError( transformationId(),
                                                           ChrysalixI18n.floorOperationInvalidTermType.text( transformationId() ) );
                    problems().add( problem );
                }
            } catch ( final ChrysalixException e ) {
                final ValidationProblem problem =
