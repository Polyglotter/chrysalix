...
public final class ArcTangent extends AbstractOperation< Double > {
    ...
    public static final ValueDescriptor< Number > TERM_DESCRIPTOR =
        TransformationFactory.createWritableBoundedOneValueDescriptor( TransformationFactory.createId( ArcTangent.class, "input" ),
                                                                       ChrysalixI18n.arcTangentOperationInputDescription.text(),
                                                                       ChrysalixI18n.arcTangentOperationInputName.text(),
                                                                       Number.class );
    ...
    public static final OperationDescriptor< Double > DESCRIPTOR =
        new AbstractOperationDescriptor< Double >( TransformationFactory.createId( ArcTangent.class ),
                                                   ChrysalixI18n.arcTangentOperationDescription.text(),
                                                   ChrysalixI18n.arcTangentOperationName.text(),
                                                   Double.class,
                                                   INPUT_DESCRIPTORS ) {
        ...
        if ( inputs().size() != 1 ) {
            final ValidationProblem problem =
                TransformationFactory.createError( transformationId(),
                                                   ChrysalixI18n.arcTangentOperationMustHaveOneTerm.text( transformationId() ) );
            ...
            try {
                ...
                if ( !( value instanceof Number ) ) {
                    final ValidationProblem problem =
                        TransformationFactory.createError( transformationId(),
                                                           ChrysalixI18n.arcTangentOperationInvalidTermType.text( transformationId() ) );
                    problems().add( problem );
                }
            } catch ( final ChrysalixException e ) {
                final ValidationProblem problem =
