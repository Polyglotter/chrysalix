...
public final class ModelerTest extends BaseTest {
    ...
    public void shouldGetDdlModel() throws Exception {
        ...
        final Model generatedModel = modeler().generateModel( new File( "src/test/resources/Teiid-MySQLAccounts.ddl" ), null,
                ARTIFACT_NAME,
                modelTypeManager().modelType(DDL_MODEL_TYPE_ID ) );
        final Model model = modeler().model( ARTIFACT_NAME );
        assertThat( model, is( generatedModel ) );
        
        for( ModelObject mo : model.children() ) {
        	System.out.println("  CHILD >>>> MO = " + mo.name());
            for( ModelObject statement : mo.children() ) {
            	System.out.println("  CHILD >>>> STATEMENT = " + statement.name());
            	//System.out.println("  CHILD >>>> STATEMENT = " + statement.name());
            	if( statement.hasProperties() ) {
            		for( String prop : statement.propertyNames() ) {
            			System.out.println("  CHILD >>>> Property = " + prop.toString());
            		}
            		String exp = (String) statement.value("ddl:expression");
            		System.out.println("  CHILD >>>> Expression = " + exp);
            	}
            	if( statement.hasChildren() ) {
                    for( ModelObject state_child : statement.children() ) {
                    	System.out.println("  CHILD >>>> StatementChild = " + state_child.name());
                    	String[] mixinTypes =  state_child.mixinTypes();
                    	for( String type : mixinTypes) {
                    		System.out.println("                 >>> MixinType = " + type);
                    	}
                    	if( state_child.hasProperties() ) {
                    		for( String prop : state_child.propertyNames() ) {
